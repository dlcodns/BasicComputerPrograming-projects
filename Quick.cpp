...
void quick(int a[], int k, int m){    //퀵을 진행하는 함수
  int i=m; //i is pivot
...
void fill(int a[], int num){    //빠진 수를 채우는 함수
  for(int j=0;j<10;j++){
    if(a[j]!=j){        //a[0]이 0이 아니면 0으로 만들고 나머지 뒤로 밀기
      for(int i=num;i>j;i--){
        a[i] = a[i-1];
      }
      a[j] = j;         //빠져있던 수 채우기
 ...
 // 개수세기
  while(1){
    ...
    for(int i=0;i<num;i++){    //배열의 범위가 맞지 않을 경우 다시 시작
      for(int j=0;j<i;j++){
        if((a[i]==a[j]) || (a[j]<0 || a[j]>9)){
          cout << "\n0 ~ 9 사이의 서로 다른 정수가 아닙니다. 다시 입력해주세요.\n\n" << endl;
          check+=1;
          j=i;    //j for문에서 나오기 위해 조건 불충분하게 만들기
